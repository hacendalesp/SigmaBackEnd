...
public class FranjasHorariasDAO  extends CrudDAO{
	...
	{ 
        ...
        parametros.add(new SentenciaParametroDAO("p_idTipoArea", OE.getIdTipoArea(), SentenciaTipoParametroDAO.ENTRADA, OracleTypes.NUMBER));
        parametros.add(new SentenciaParametroDAO("p_validarDistribucion",  OE.isValidarDistribucion() ? 1 : 0 , SentenciaTipoParametroDAO.ENTRADA, OracleTypes.NUMBER));
        
        
        ObjetoSalida os = ejecutar(OE, "PKG_PROGRAMACION"+".prc_generar_franjas_recurso", parametros); 
        
        if(os.getCodError() == CodError.PROCESADO_SIN_RESULTADO)
