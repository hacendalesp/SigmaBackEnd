...
public class GestionPendienteServicio implements IGestionPendienteServicio {
	...
	{
		OE.setIdActividad(4L); //para revisar visita diagnostico
	...
	{
		OE.setIdActividad(5L); //para validar visita diagnostico
	...
	@Override
	public Response avanzarMantenimientosVialesMasivoGenerarFicha(GestionOE OE) {
		GestionDTO gestion = new GestionDTO();
		OE.setTipoGestion(CodTipoGestion.AVANZAR);
		ObjetoSalida os = gestion.avanzarMantenimientosVialesMasivoGenerarFicha(OE);
		return Response.status(Response.Status.OK).entity(os).build();
		
	}

	@Override
	public Response listarPanelGestionPendienteTarea(GestionOE OE) {
		IGestionDTO consulta = new GestionDTO();
		ObjetoSalida os = consulta.listarPanelPendiente(OE);
		return Response.status(Response.Status.OK).entity(os).build();
	}
	...
	@Override
	public Response listarBandejaGestionPendienteTarea(GestionOE OE) {
		IGestionDTO consulta = new GestionDTO();
		ObjetoSalida os = consulta.listarGestionPendienteTarea(OE);
		return Response.status(Response.Status.OK).entity(os).build();
	}
