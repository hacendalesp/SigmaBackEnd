...
public class GestionDTO implements IGestionDTO 
{
	...
	public ObjetoSalida avanzarMantenimientosVialesMasivoGenerarFicha(GestionOE OE) {
		ObjetoSalida res = new ObjetoSalida();
		...
		{			
			IGestionDTO consulta = new GestionDTO();
			ReporteDTO dtoReporte = new ReporteDTO();
			ObjetoSalida os = consulta.obtenerGestionPendientePorIdTransicion(OE);
			...
			if(os.esRespuestaOperacionCorrecta()) {				
				...
				for(HashMap<String, Object> map : os.getRespuesta())
				{
					...
					for(Integer id : OE.getIds()) 
					{
						System.out.println(idProcesoGestion + " = " + id);
						if(id.equals(idProcesoGestion))
						{
							LinkedHashMap<String, Object> registro = new LinkedHashMap<String, Object>(2);	
							Long id_mantenimiento_vial = Long.parseLong(os.getRespuesta().get(index).get("id_mantenimiento_vial").toString());
							Long id_mantenimiento_vial_evento = Long.parseLong(os.getRespuesta().get(index).get("id_mantenimiento_vial_evento").toString());
							Long id_documento = Long.parseLong(os.getRespuesta().get(index).get("id_documento").toString());
							
							String extent = "export?bbox=-8281812.2,496938.2,-8219704.1,539396.1";
							String bboxSR = "102100"; 
							...
							ObjetoSalida osProc = new ObjetoSalida();
							try
							{
								ExternoDTO pasarela = new ExternoDTO();
								
								ObjetoSalida pas = pasarela.consumirExterno(obj);
								osProc =  extraerObjetoSalida(pas);
								
								if(osProc.esRespuestaOperacionCorrecta()) {
									registro.put("rtaGestion", os.getCodError());
									registro.put("msgGestion", os.getMsgError());
									registro.put("id_proceso_gestion", id);
									
									if(OE.getIdActividad() != null) {
										boolean guardar = true;
										GenerarFichaOE OEReporte = new GenerarFichaOE();
										OEReporte.setIdActividad(OE.getIdActividad());
										OEReporte.setIdMantenimientoVialEvento(id_mantenimiento_vial_evento);
										OEReporte.setIdMantenimientoVial(id_mantenimiento_vial);
										OEReporte.setIdDocumento(id_documento);
										OEReporte.setBboxSR(bboxSR);
										OEReporte.setUsuario(OE.getUsuario());
										OEReporte.setExtent(extent);
										OEReporte.setGuardar(guardar);
										ObjetoSalida osReporte = dtoReporte.generarFichaWordSinArchivo(OEReporte);
										System.out.println(registro.toString());
										System.out.println(osReporte.getCodError());
										System.out.println(osReporte.getMsgError());
										if(osReporte.esRespuestaOperacionCorrecta()) {
											registro.put("respuesta",osReporte);
										}
										//break;
									}
									respuesta.add(registro);
							...
							{
								if(osProc.getRespuesta() == null)
								{
									registro.put("rtaGestion", CodError.ERROR_INTERNO);
									
									if(osProc.getMsgError() == null) 
									{
										registro.put("msgGestion", "No se obtuvo una respuesta correcta de Caliope");
									}
									else
	...
	@Override
	public ObjetoSalida listarPanelPendienteTarea(GestionOE OE) {
		IConsultaGestionDAO modelo = new ConsultaGestionDAO();
		return modelo.listarPanelPendienteTarea(OE);	}

	@Override
	public ObjetoSalida listarGestionPendienteTarea(GestionOE OE) {
		IConsultaGestionDAO modelo = new ConsultaGestionDAO();
		return modelo.listarGestionPendienteTarea(OE);
	}
}
